...
export default function Login(props) {
	...
	const loginErrorContainer = (
		<ErrorMessage message={"No active account found with the given credentials."} />
	);
	...
	const handleLogin = async (e) => {
		...
		if (email && password && isEmailValid) {
			// make the api call to check user credentials
			try {
	...
	useEffect(() => {
		if (auth.token && !auth.errorMessage) {
	...
	return (
		<>
			<CSSTransition in={props.display} unmountOnExit timeout={500} classNames={"menu-login"}>
				<aside className={`login-form login-form-opened  shadow`}>
					<button
						className={`login-close-button close-button`}
					...
					<button
						className={`login-btn uni-button w100`}
						type="submit"
						onClick={handleLogin}
					>
						Sign in
