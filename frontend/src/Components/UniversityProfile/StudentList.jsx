...
function UserBar(props) {
	return (
		<>
			<Link
				to={`/profile/${urlToPk(props.url)}`}
				className={`${LocalCSS.UserBar} basic-list-item shadow`}
			>
	...
	async function retrieveNonStudents() {
		...
		try {
			const data = await apiCall(
				`users?search=${search}&not_student_of=${props.entityUrl}`,
				token,
				params
			);
			const newUsers = [];
			...
			const insert_user = async (user_url, key) => {
				...
				newUsers.push(
					<UserBar
						...
						key={key}
						seeApplications={props.seeApplications}
						setApplicationsToggle={props.setApplicationsToggle}
						{...info}
					/>
				);
			};
