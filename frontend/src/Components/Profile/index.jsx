...
function Profile(props) {
	...
	const [popupClasses1, setPopupClasses1] = useReducer(changePopupClasses, [
		"popup-closed",
		"hidden",
	]);
	...
	const [popupClasses5, setPopupClasses5] = useReducer(changePopupClasses, [
		"popup-closed",
		"hidden",
	]);
	const [popupClasses6, setPopupClasses6] = useReducer(changePopupClasses, [
		"popup-closed",
		"hidden",
	]);
	const [popupClasses7, setPopupClasses7] = useReducer(changePopupClasses, [
		"popup-closed",
		"hidden",
	]);
	...
	if (
		popupClasses1[1] === "hidden" &&
		popupClasses2[1] === "hidden" &&
		popupClasses3[1] === "hidden" &&
		popupClasses4[1] === "hidden" &&
		popupClasses5[1] === "hidden" &&
		popupClasses6[1] === "hidden" &&
	...
	// dont remember
	useEffect(() => {
		setEditable(urlToPk(userData.url) === urlParams.id);
	}, [userData, urlParams]);
	...
	useEffect(async () => {
		...
		apiCall(`user-data?user=${targetUser.url}&`, token, { method: "GET" }).then((data) => {
			...
			let newCertificationItems = info["certifications"].map((entry, index) => {
				...
				if (entry.proof) {
					proof = (
						<a href={entry.proof} target="_blank" rel="noopener noreferrer">
							Proof
						</a>
					);
				}
				const subtitle = proof ? (
					<p>
						{proof} <br /> {entry.description}
					</p>
				) : (
					entry.description
				);
		...
		apiCall(`posts?user=${targetUser.url}&`, token, { method: "GET" }).then((data) => {
			...
			for (const post of posts) {
				newPostItems.push(
					<PostCommentContainer content={post.content} key={postCommentIdx} />
				);
				postCommentIdx++;
			}
		...
		apiCall(`comments?user=${targetUser.url}&`, token, { method: "GET" }).then((data) => {
			...
			for (const comment of comments) {
				newCommentItems.push(
					<PostCommentContainer content={comment.content} key={postCommentIdx} />
				);
				postCommentIdx++;
			}
	...
	function updateBasicInfo(e) {
		...
		apiCall(userInformation.url, token, params)
			.then((data) => {
	...
	function createNewEducation(e) {
		...
		apiCall("education-data", token, params)
			.catch((error) => {
				alert(error);
			})
			.then((data) => {
				...
				let newItem = (
					<ProfileContentItem type="education" {...data} key={educationItems.length} />
				);
	...
	function createNewExperience(e) {
		...
		apiCall("experience-data", token, params)
			.catch((error) => {
				alert(error);
			})
			.then((data) => {
				...
				let newItem = (
					<ProfileContentItem type="experience" {...data} key={experienceItems.length} />
				);
	...
	function createNewSkill(e) {
		...
		apiCall("skill-data", token, params)
			.catch((error) => {
				alert(error);
			})
			.then((data) => {
			...
			<div className={`main-content`}>
				<div className={`profile-user`}>
					<PopupForm
						...
						popupClasses={popupClasses1}
						setPopupClasses={setPopupClasses1}
						handleSubmit={updateBasicInfo}
					>
						<BasicInput
							name="edit-info-headline"
							type="text"
							width="100%"
							label="Headline"
							value={editInfoHeadline}
							setter={setEditInfoHeadline}
							required="no"
						/>

						<div className={`double-input-wrap w100 flex row-wrap j-c-s-b a-i-c`}>
							<BasicInput
								name="edit-info-location"
								type="text"
								width="47%"
								label="Location"
								value={editInfoLocation}
								setter={setEditInfoLocation}
								required="no"
							/>
							<BasicInput
								name="edit-info-website"
								type="text"
								width="47%"
								label="Website"
								value={editInfoWebsite}
								setter={setEditInfoWebsite}
								required="no"
							/>
						</div>
						...
						<TextArea
							width="100%"
							label="Summary"
							value={editInfoSummary}
							setter={setEditInfoSummary}
							rows="5"
							menuTop="20%"
							required="no"
						/>
					...
					>
						<BasicInput
							...
							width="100%"
							label="Title"
							value={externalProfileTitle}
							setter={setExternalProfileTitle}
							required="yes"
						/>
						<BasicInput
							type="text"
					...
					>
						<BasicInput
							name="add-education-institution"
							type="text"
							width="100%"
							label="Institution"
							value={eduInstitution}
							setter={setEduInstitution}
							required="yes"
						/>
						<BasicInput
							name="add-education-degree"
							type="text"
							width="100%"
							label="Degree"
							value={eduDegree}
					...
					>
						...
						<BasicInput
							...
							label="Description"
							value={courseDesc}
							setter={setCourseDesc}
							required="yes"
						/>
						<BasicInput
							name="add-skill-skill"
							type="text"
							width="100%"
							label="ECTS (optional)"
							value={ects}
					...
					<PopupForm
						...
						popupClasses={popupClasses5}
						setPopupClasses={setPopupClasses5}
					...
					>
						...
						<BasicInput
							...
							setter={setCertificationDescription}
							required="yes"
						/>
						<AttachImage
					...
					<PopupForm
						...
						popupClasses={popupClasses6}
						setPopupClasses={setPopupClasses6}
						handleSubmit={createNewExperience}
					>
						<BasicInput
							name="add-experience-company"
							type="text"
							width="100%"
							label="Company"
							value={expCompany}
							setter={setExpCompany}
							required="yes"
						/>
						<BasicInput
							name="add-experience-job-title"
							type="text"
							width="100%"
							label="Job Title"
							value={expJTitle}
						...
						<div className={`double-input-wrap w100 flex row-wrap j-c-s-b a-i-c`}>
							<BasicInput
								name="add-experience-start-date"
								type="date"
								width="47%"
								label="Start Date"
								value={expSDate}
					...
					<PopupForm
						title="Add Skill"
						popupClasses={popupClasses7}
						setPopupClasses={setPopupClasses7}
					...
					>
						...
						<BasicInput
							...
							label="Skill"
							value={skillSkill}
							setter={setSkillSkill}
							required="yes"
						/>
					</PopupForm>

					<div className={`main-profile-menu-user`}>
						<div className={`fixed-menu-user`}>
							<NavLink
								to="#basic-info"
								className={`profile-menu-item-user ${menuClassesArray[0]}`}
							...
							<NavLink
								to="#education"
								className={`profile-menu-item-user ${menuClassesArray[1]}`}
							...
							<NavLink
								to="#experience"
								className={`profile-menu-item-user ${menuClassesArray[2]}`}
								onClick={(e) => changeActiveItem(2)}
							>
								<div className={`item-text`}>Experience</div>
							</NavLink>

							{/*<NavLink to="#university-courses" className={`profile-menu-item-user ${menuClassesArray[3]}`}*/}
							{/*    onClick={e => changeActiveItem(3)}>*/}
							{/*    <div className={`item-text`}>University Courses</div>*/}
							{/*</NavLink>*/}

							{/*<NavLink to="#certifications" className={`profile-menu-item-user ${menuClassesArray[4]}`}*/}
							...
							<NavLink
								to="#community"
								className={`profile-menu-item-user ${menuClassesArray[5]}`}
								onClick={(e) => changeActiveItem(3)}
							>
								<div className={`item-text`}>Community</div>
							</NavLink>

							{/*<NavLink to="#skills" className={`profile-menu-item-user ${menuClassesArray[6]}`}*/}
							{/*    onClick={e => changeActiveItem(6)}>*/}
							{/*    <div className={`item-text`}>Skills</div>*/}
							{/*</NavLink>*/}

							{/*<NavLink to="#posts" className={`profile-menu-item-user ${menuClassesArray[7]}`}*/}
							{/*    onClick={e => changeActiveItem(7)}>*/}
							{/*    <div className={`item-text`}>Posts</div>*/}
							{/*</NavLink>*/}

							{/*<NavLink to="#comments" className={`profile-menu-item-user ${menuClassesArray[8]}`}*/}
					...
					<div className={`profile-content-container-user`}>
						<ProfileContentFrame
							id="basic-info"
						...
						>
							<div className={`profile-banner-user`}>
								<img
									className={`profile-banner-pfp-user`}
							...
							<div className={`profile-basic-info`}>
								{editable && (
									<img
										className={`basic-info-toggler-user`}
										src={pencil_icon}
										alt="Pen icon"
										onClick={setPopupClasses1}
									/>
								)}
								<h3>
						...
						>
							...
							{universityCourseItems.length === 0 && (
								<h4 className={`normal`} style={{ margin: "1rem 0" }}>
									You haven't added any experience yet...
								</h4>
							)}
