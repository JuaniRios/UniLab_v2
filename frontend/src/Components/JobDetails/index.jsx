...
export default function JobDetails(props) {
	...
	useEffect(async () => {
		...
		setJobData(job_data);
		const company_data = await apiCall(job_data.company.url, token, {
			method: "GET",
			fullUrl: true,
		});
	...
	return (
		<>
			...
			<div className={`main-content-fixed-2`}>
				<Link to={"/jobs"} className={`return-btn`}>
					&#8617; Return to Jobs
				</Link>
				<div className={`job-details shadow`}>
					<div className={`job-details-left`}>
						{companyData && (
							<Link to={`/companies/${urlToPk(companyData.url)}`}>
								<img className={`job-company-img`} src={companyData.image} />
							</Link>
						)}
						<div className={`job-mini-btns-holder`}>
							{jobData.website_url && (
								<a className={`job-mini-btn shadow`} href={jobData.website_url}>
									Website
								</a>
							)}
						</div>
						<h3 className={`job-info-text`}>{companyData.name}</h3>
						<div className={`job-info-text`}>{companyData.employee_range_verbose}</div>
						<hr className="hr100" />
					...
					<div className={`job-details-right`}>
						...
						<div className={`job-info-container`}>
							<div className={`job-info-subcontainer`}>
								...
								<div className={`job-info-text`}>
									<b>{jobData.salary_per_month} EUR</b>
								</div>
							</div>
							<div className={`job-info-subcontainer`}>
								<div className={`job-info-text`}>Weekly Hours</div>
								<div className={`job-info-text`}>
									<b>{jobData.hours_per_week} hours</b>
								</div>
							</div>
						</div>
						<hr className="hr100" />
						...
						<div className={`job-info-longer-text w100`}>
							<h3>What we offer</h3>
							<p>{jobData.we_offer}</p>
						</div>
					</div>
				</div>
				<button
					className={`apply-btn uni-button`}
					type="button"
			...
			{applyForm ? (
				...
				>
					...
					<AttachImage
						...
						setSpanText={setSpanText}
					/>
					<h4 className="w100">Other Files</h4>
					<AttachImage
						label="Choose files"
						multiple={true}
						document={true}
					...
					<h4>
						This company will also have access to your profile when you apply. We
						recommend that you finish putting your profile up to date before applying.
					</h4>
				</PopupForm>
			) : null}
		</>
