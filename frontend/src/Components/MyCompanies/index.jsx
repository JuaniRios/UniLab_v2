		...
		if (userData) {
			try {
				...
				for (const url of userData.company_admins) {
					...
					newCompanies.push(await apiCall(admin_data.company, token, {method: "GET", fullUrl: true}))
				}

				// get full data of each company
				// build widget
				for (const [i, data] of newCompanies.entries()) {
					setCompanies(prevState => prevState.concat(
						<CompanyItem key={i} {...data} url={`/companies/${urlToPk(data.url)}`}/>
					))
				}
			} catch (e) {
