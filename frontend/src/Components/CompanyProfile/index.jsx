...
export default function CompanyProfile(props) {
    ...
    useEffect( async () => {
        ...
        try {
            let images = await apiCall(`company-pictures?owner=${data.url}`, token, {method: "GET"})
            ...
            <div className={`profile`}>
                ...
                <div className={`profile-content-container`}>
                    ...
                    <ProfileContentFrame id="skills" className={`${contentClassesArray[3]}`} margin={true} title="Admins"
                        plusBtn={false} onClick={setPopupClasses4}>

                        <CSSTransition in={companyData} unmountOnExit timeout={500}
                                       classNames={"admin-list-transition"}>
                            <div>
                                <div className={`add-new-field custom-scroll`}>
                                    <AdminList title={`Manage admins:`} option="REMOVE"
                                               forceReload={[forceReload, setForceReload]}
                                               companyUrl={companyData.url}/>

                                </div>
                                <div className={`add-new-field custom-scroll`}>
                                    <AdminList title="Add a new admin:" option="ADD"
                                               forceReload={[forceReload, setForceReload]}
                                               companyUrl={companyData.url}/>
                                </div>
                            </div>
